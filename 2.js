...
const sum = (list, fn)=>list.reduce((result, num)=>result+Number(fn(num)), 0);
const max = (list)=>list.reduce((result, num)=>(num > result ? num : result), -Infinity);
...
const findDelta = (list)=>{
	const {max, min} = list.reduce((acc, num)=>{
...
const findMaxDivider = (list)=>{
	const doesDivide = (a, b)=>{
		if(b > a) [a,b] = [b,a];
		return a % b == 0 ? a/b : 0;
	};
	return max(mapPairs(list, doesDivide));
};

const checksum = (input, fn)=>sum(process(input), (line)=>fn(line));

console.log('Part 1', checksum(input, findDelta));
